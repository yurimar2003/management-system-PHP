...
$objPdf->Ln(20);
	...
	$objPdf->Cell(40,8,utf8_decode("Apellido"),1,0,'C',true);
	$objPdf->Cell(10,8,utf8_decode("Nac."),1,0,'C',true);
	$objPdf->Cell(25,8,utf8_decode("Doc. Ident."),1,0,'C',true);
	$objPdf->Cell(23,8,utf8_decode("Cargo"),1,0,'C',true);
	$objPdf->Cell(18,8,utf8_decode("Fecha"),1,0,'C',true);
	$objPdf->Cell(10,8,utf8_decode("Sexo"),1,0,'C',true);
	$objPdf->Cell(45,8,utf8_decode("Direccion"),1,0,'C',true);
	$objPdf->Cell(25,8,utf8_decode("Telefono"),1,0,'C',true);
	$objPdf->Ln(8);	
	$objPdf->SetTextColor(39,39,51);

	/*----------  Detalles de la tabla  ----------*/
	while($fila = $empleados->fetch_assoc()) {
	$objPdf->Cell(8, 7, utf8_decode($fila['id_empleado']),'L',0,'C');
...
/* 	$objCell->CellFitScale(20, 7,utf8_decode($fila['apellido']),'L',0,'C'); */
	...
	$objPdf->Cell(23, 7, utf8_decode($fila['cargo']),'L',0,'C');
	$objPdf->Cell(18, 7, utf8_decode($fila['fecha_nacimiento']),'L',0,'C');
	$objPdf->Cell(10, 7, utf8_decode(substr(($fila['sexo']), 0, 3)),'L',0,'C'); 	
	$objPdf->Cell(45, 7, utf8_decode(substr(($fila['direccion']), 0, 25)),'L',0,'C'); 
